...
if [ ! -f "./experiments/solr17515/conf/solr.properties" ]; then
    echo "Error: solr.properties file does not exist"
    exit 1
fi

echo "Executing: . ./run_engine.sh transform ../conf/solr.properties"
. ./run_engine.sh transform ./experiments/solr17515/conf/solr.properties
...
rm -rf "$HOME/solr_instrumented"
mkdir -p "$HOME/solr_instrumented"
cp -r "$LOCAL_PATH" "$HOME/solr_instrumented/"
