package edu.uva.liftlab.pilot.option;
...
import edu.uva.liftlab.pilot.util.StringUtils;
import edu.uva.liftlab.pilot.RCMain;

/**
 * A parser for the rc command line arguments
 */
public class OptionParser {
    ...
    private Option mainClass = Option.builder("m").longOpt("main").hasArg()
            .desc("PILOTMain class of package").build();
    ...
    private Option listAnalysis = Option.builder().longOpt("list")
            .desc("List analysis available in PILOT").build();
    ...
    private Option configFile = Option.builder("C").longOpt("systemConfigFile").hasArg().argName("file")
            .desc("Configuration file that contains the settings for the code to be analysed").build();
    ...
    public RCOptions parse(String[] args) throws OptionError {
        try {
            CommandLine cmd = new DefaultParser().parse(mOptions, args);
            RCOptions options = RCOptions.getInstance();
            ...
            String path = RCMain.class.getProtectionDomain().getCodeSource().getLocation()
                    .getPath();
            ...
            LOG.info("configFile path is: " + cmd.getOptionValue(configFile.getLongOpt()));
            options.setConfigFilePath(cmd.getOptionValue(configFile.getLongOpt()));
    ...
    public void printHelp() {
        ...
        formatter.printHelp(80, "PILOT.jar [OPTIONS] -- [SOOT OPTIONS]",
                null, mOptions, null, false);
    }

    public void listAnalyses() {
        System.out.println("Available PILOT analyses:");
